...
void UART_Transmit(UART_HandleTypeDef *huart, uint8_t *data, uint16_t size) {
    for (uint16_t i = 0; i < size; i++) {
        ...
        huart->Instance->DR = data[i]; // Transmit byte
    }
...
int main(void)
{
  ...
  {
	  char str[] = "FaraDars\n";
	  UART_Transmit(&huart2, (uint8_t*)str, strlen(str));
	  HAL_Delay(500);
  }
}
...
void vTask1(void* pvParameters)
{
	char *Message1 = "Hello, my name is Task1\n";
	while(1)
	{
		xSemaphoreTake(myMutex, portMAX_DELAY);
		UART_Transmit(&huart2, (uint8_t*)Message1, strlen(Message1));
...
void vTask2(void* pvParameters)
{
	char *Message2 = "nice to meet you I am Task2\n";
	while(1)
	{
		xSemaphoreTake(myMutex, portMAX_DELAY);
