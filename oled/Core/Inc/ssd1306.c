...
void ssd1306_FillCircle(uint8_t par_x, uint8_t par_y, uint8_t par_r, SSD1306_COLOR par_color) {
    ...
    // ??? ?? ????? ?? ??? ?????
    for (int i = par_y - par_r; i <= par_y + par_r; i++) {
        ssd1306_DrawPixel(par_x, i, par_color);
    }

    while (y >= x) {
        // ???? ?? ????? ?? ???? ?????? ???? ???? ??? ???????? ??? ???????
        for (int i = par_x - x; i <= par_x + x; i++) {
            ssd1306_DrawPixel(i, par_y + y, par_color);
            ssd1306_DrawPixel(i, par_y - y, par_color);
        }
        for (int i = par_x - y; i <= par_x + y; i++) {
            ssd1306_DrawPixel(i, par_y + x, par_color);
            ssd1306_DrawPixel(i, par_y - x, par_color);
        }
